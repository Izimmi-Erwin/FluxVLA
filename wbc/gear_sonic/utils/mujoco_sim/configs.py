...
class BaseConfig(ArgsConfigTemplate):
    ...
    backend: Optional[Literal["mujoco", "isaac"]] = None
    """Alias for simulator. Example: --backend isaac."""
    ...
    def __post_init__(self):
        if self.backend is not None:
            self.simulator = self.backend
        else:
            self.backend = self.simulator  # keep CLI/config dumps explicit
...
class SimLoopConfig(BaseConfig):
    ...
    isaac_publish_camera: bool = True
    """Publish an Isaac camera stream using the same 5555 protocol as MuJoCo."""

    isaac_publish_debug_state: bool = False
    """Optional Isaac-side 5557 g1_debug publisher; keep False when GR00T C++ is running."""
    ...
    isaac_elastic_band_enabled: bool = True
    """Start Isaac with ElasticBand enabled, matching the MuJoCo startup flow."""

    state_zmq_port: int = 5557
    """Port for the optional Isaac-side g1_debug state publisher."""

    state_zmq_topic: str = "g1_debug"
    """Topic prefix for the optional Isaac-side g1_debug state publisher."""

    enable_eval_control: bool = False
    """Enable the ZMQ control server used by automated simulation evaluation"""

    eval_control_host: str = "127.0.0.1"
    """Host/interface for the automated evaluation control server"""

    eval_control_port: int = 5590
    """Port for the automated evaluation control server"""
